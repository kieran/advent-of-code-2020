...
## Defs
    ...
    class Rule
      ...
      first_invalid_field: (ticket=[])=>
        for num in ticket
          return num unless @num_valid num
        null

    find_invalid_values = (rules=[], tickets=[])->
      invalid_values = []
      for t in tickets
        for val in t
          if not any rules.map (r)-> r.num_valid val
            invalid_values.push val
      invalid_values

    sumArray = (array=[])-> array.reduce (memo=0, val=0)-> memo + val

    error_rate = (rules=[], tickets=[])->
      sumArray find_invalid_values rules, tickets
...
## Tests
    ...
    assert 71 is error_rate ...parseInput input

## Run

    log error_rate ...parseInput require './input'
