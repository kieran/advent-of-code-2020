...
## Defs
    ...
    playGame = ([d1, d2], seen=[])->
      until 0 is d1.length or 0 is d2.length
        ...
        round_key = [d1.join(','),d2.join(',')].join('-')

        # if player 1 wins, only their deck should be scored
        # so we remove the deck for player 2
        return [d1,[]] if round_key in seen
        seen.push round_key
        ...
        # should we recurse?
        if card1 <= d1.length and card2 <= d2.length
          [r1, r2] = playGame [ d1[0...card1], d2[0...card2] ]
          if r1.length
            d1.push card1, card2
          else
            d2.push card2, card1
        else
          # simple case - higher card wins
          if card1 > card2
            d1.push card1, card2
          else
            d2.push card2, card1
...
## Tests
    ...
    assert 291 is scoreDeck playGame parseInput input

## Run

    log scoreDeck playGame parseInput require './input'
