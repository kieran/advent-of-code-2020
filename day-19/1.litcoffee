...
## Tests
    ...
    [ rules, data ] = parseInput input

    r = regexify processRules(rules)[0]
    assert r.test 'ababbb'
    assert not r.test 'bababa'
    assert r.test 'abbbab'
    assert not r.test 'aaabbb'
    assert not r.test 'aaaabbb'

    assert 2 is countValid ...parseInput input

## Run

    log countValid ...parseInput require './input'
